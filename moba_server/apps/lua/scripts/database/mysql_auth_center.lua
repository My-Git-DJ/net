...
function mysql_connect_to_auth_center()
	local auth_conf = game_config.auth_center
...
function insert_guest_user(g_key, ret_handler)
	...
	local sql = "insert into uinfo(guest_key,unick,uface,usex,is_guest)value(\"%s\",\"%s\",%d,%d,1)"
...
function edit_profile(uid, unick, uface, usex, ret_handler)
	...
	Mysql.query(mysql_conn, sql_cmd, ret_handler)
end
