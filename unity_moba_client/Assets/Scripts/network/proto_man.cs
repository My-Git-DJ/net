...
public class cmd_msg
{
    public int stype;
    public int ctype;
    public byte[] body;//protobuf or utf8 string
}

public class proto_man {
    private const int HEADER_SIZE = 8;//2-stype,2-ctpye,4-utag,msg->body

    private static byte[] protobuf_seriallizer(ProtoBuf.IExtensible data)
    {
    ...
    public static byte[] pack_protobuf_cmd(int stype,int ctype,ProtoBuf.IExtensible msg)
    {
        ...
        if (msg != null)
        {
            cmd_body = protobuf_seriallizer(msg);
            cmd_len += cmd_body.Length;
        }
        byte[] cmd = new byte[cmd_len];
        //stype,ctype,utag(保留),cmd_body
